...
public class ChatRoomFrame extends Thread implements ActionListener {
	...
	// 构造方法
	public ChatRoomFrame(String name, String ip) {
		strServerIp = ip;
		strLoginName = name;
		frmChat = new JFrame("用户:" + name);
		pnlChat = new JPanel();
		frmChat.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmChat.getContentPane().add(pnlChat);

		Font fntDisp1 = new Font("宋体", Font.PLAIN, 16);
		Font fntDisp2 = new Font("宋体", Font.PLAIN, 25);
		String list[] = { "所有人" };
		...
		taUserMessage = new TextArea("", 320, 550,
		...
		pnlChat.setBackground(new Color(176, 196, 222));
		
		btnSave.setBounds(450, 550, 80, 25);
		btnCls.setBounds(550, 550, 80, 25);
		btnExit.setBounds(650, 550, 80, 25);
		btnSend.setBounds(800, 550, 80, 25);
		...
		lblChatUser.setBounds(400, 0, 40, 40);
	//	lblSendMessage.setBounds(210, 290, 60, 40);

		lstUserList.setBounds(5, 40, 320, 200);
		lstUserList.setFont(fntDisp2);
		
		taUserMessage.setBounds(330, 40, 570, 400);
		txtMessage.setBounds(330, 440, 550, 100);
		cmbUser.setBounds(455, 5, 80, 25);
	...
	//	pnlChat.add(lblSendMessage);
		...
		headLabel.setIcon(new ImageIcon("images//Mess.jpg"));
		headLabel.setBounds(0, 240, 330, 350);
		...
		Image img = toolkit.getImage("face//5.JPG");
				...
				for (int i = intMessageCounter; i < messobj.chat.size(); i++) {
					...
					if (temp.chatUser.equals(strLoginName)) {
						if (temp.chatToUser.equals(strLoginName)) {
							...
							{
								temp_message = "[我]对[" + temp.chatToUser + "]"
										+ "说：" + temp.chatMessage
										+ "\n";
							} else {
								temp_message = "[我]悄悄对[" + temp.chatToUser
										+ "]" + "说：" + temp.chatMessage
										+ "\n";
							}
						}
						...
						if (temp.chatToUser.equals(strLoginName)) {
							if (!temp.whisper) // 不是悄悄话
							{
								temp_message = "[" + temp.chatUser + "]对[我]"
										+ "说：" + temp.chatMessage
										+ "\n";
							} else {
								temp_message = "[" + temp.chatUser + "]悄悄对[我]"
										+ "说：" + temp.chatMessage
										+ "\n";
							}
						} else {
	...
	public static void main(String args[]) {
		new ChatRoomFrame("测试用户", "127.0.0.1");
	}
